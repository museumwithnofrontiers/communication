...
export default {
  ...
  // addresses every DXA config repeats, communication adding none of its own.
  links: { ...mwnfLinks },

  // The source-database chip's colour, per project (epic #1727 phase 4):
  // one entry for every project id `npm pack @museumwnf/communication-data`
  // carries in `manifest.projects` (checked 2026-09-22, communication-data
  // 1.0.1). The class names are `@museumwnf/viewer-layout`'s own fixed chip
  // palette (`.mwnf-chip--<name>`, themed by its `--mwnf-project-<name>`
  // tokens) — this site just says which project gets which one, by id,
  // instead of computing it from a legacy project key at runtime. Comments
  // name the project the way the manifest itself does
  // (`manifest.projects[id].name.en`).
  //
  // "The Hijaz Railway" (e08d1c71-8cff-5fe9-b480-2ae6c530b732) has no family
  // among the layout's fixed chip palette (only AWE/DBA/DCA/DGA/EXH/
  // Galleries/ISLandEPM exist) — it defaults to `mwnf-chip--Galleries` here,
  // an open platform question raised in this PR's body rather than silently
  // resolved.
  projectColors: {
    ...
    'e08d1c71-8cff-5fe9-b480-2ae6c530b732': 'mwnf-chip--Galleries', // The Hijaz Railway — no fixed chip family exists for it; defaulted, see PR body
  },
  ...
  // own editorial choice, not a literal legacy key check.
  ...
  site: { origin: 'https://museumwithnofrontiers.github.io/communication' },
  ...
  extraViews: [
    ...
    // `body` entry name).
    ...standardRoutes('gallery', { creditsBody: 'communication.credits.body' }),
  ],
