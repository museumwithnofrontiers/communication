...
describe('website smoke test', () => {
  ...
  it('renders the collection results on the composed results view', async () => {
    ...
    // Nine a page; 126 items across the roster this gallery borrows means
    // more than one pagination control renders (top and bottom).
  ...
  it('renders a keyword search on the composed results view', async () => {
    ...
    // items[0]'s own name opens with a Markdown-italic asterisk
    // (`*Algiers port*`), which is not a clean single search word — the
    // borrowed Discover Islamic Art record's own plain name is used instead;
    // its own internal name is always in its own haystack, so searching for
    // its first word is still guaranteed at least one hit.
    const item = items.find((i) => i.id === 'f542a798-8c47-5278-ad25-50059baf16a0')
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    // Communication and Transportation owns no native project of its own —
    // every one of its 126 items is borrowed from six other MWNF projects —
    // so this site's own `gallery.about.body` names the three it draws most
    // of its roster from, rather than a single native-project name.
    expect(host.textContent).toContain('Sharing History')
  ...
  it('renders the item sheet on the composed record view', async () => {
    ...
    // belongs to "Sharing History - Arab-Ottoman-European relations in the
    // 19th century.", the project this borrows-only gallery draws the most
    // items from (99 of 126).
  ...
  // project other than this gallery's own plurality project.
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/f542a798-8c47-5278-ad25-50059baf16a0')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/2735c542-1b56-59a7-9fac-72518fe82535')
    ...
    const isl = await mountSite('#/item/f542a798-8c47-5278-ad25-50059baf16a0')
  ...
  // that project's URL is non-null. communication-data 1.0.1 carries both
  // URLs for Discover Islamic Art, the borrowed ISL record's project, so this
  // asserts the positive case against the package's own values rather than a
  // hard-coded URL.
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    // The Hijaz Railway blueprint map (a borrowed Explore Islamic Art
    // Collections record) with the Ottoman dynasty attached — a record whose
    // dynasty translation actually carries a history, so it survives
    // ItemSheet.vue's own "has something to show" filter.
    const { app, host } = await mountSite('#/item/2735c542-1b56-59a7-9fac-72518fe82535')
  ...
  it('renders the timeline results on the composed timeline view', async () => {
    const { app, host } = await mountSite('#/timeline-results?country=rm')
    // The summary counts every matching event (18); three of Romania's own
    // events have no English translation and are silently dropped from the
    // rendered list rather than shown untranslated, so only 15 rows render.
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-timeline__row').length).toBe(15), { timeout: 20000 })
    expect(host.querySelector('.mwnf-summary').textContent).toContain('18')
    // A Romania event's own description, not just a row count — its
    // translation loads asynchronously, behind the spec's own `tr`.
    await vi.waitFor(() => expect(host.textContent).toContain('Alexandru Ioan Cuza'), { timeout: 20000 })
    // The join to member items finds Romania's five dated objects even with
    // no period chosen — wave 0's "all countries too" behaviour, kept.
    expect(host.querySelector('.mwnf-timeline__gallery').textContent).toContain('5')
    app.unmount()
  }, 60000)

  it('renders the timeline gallery on the composed results view', async () => {
    const { app, host } = await mountSite('#/timeline/gallery?country=rm')
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-grid__tile').length).toBe(5), { timeout: 20000 })
    expect(host.querySelector('.mwnf-summary').textContent).toContain('Romania')
    expect(host.textContent).toContain('Brăila')
    app.unmount()
  }, 60000)

  // metanull/carpets#42: the composed entrance's own country control writes
  // the inventory id (`rou`), not the legacy two-letter code (`rm`) the
  // gallery's own deep links carry — `countryIdForCode` used to answer
  // nothing for an id, so every event matched instead of Romania's own. Both
  ...
  it('filters the timeline the same way on the inventory id as on the legacy code', async () => {
    const { app, host } = await mountSite('#/timeline-results?country=rou')
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-timeline__row').length).toBe(15), { timeout: 20000 })
    expect(host.querySelector('.mwnf-summary').textContent).toContain('18')
    await vi.waitFor(() => expect(host.textContent).toContain('Alexandru Ioan Cuza'), { timeout: 20000 })
    expect(host.querySelector('.mwnf-timeline__gallery').textContent).toContain('5')
  ...
  it("submits the timeline entrance's country control (an id) to a results address that actually filters", async () => {
    ...
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-timeline__row').length).toBe(15), { timeout: 20000 })
    expect(host.querySelector('.mwnf-timeline__caption').textContent).toContain('Romania')
    app.unmount()
  }, 60000)
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    // communication-data 1.0.1 carries exactly one featured partner.
    expect(host.textContent).toContain('Türkiye')
    expect(host.textContent).toContain('Museum of Turkish and Islamic Arts')
    // The object count is this website's own copy over the shared entry, not
    // a generic count.
    expect(host.textContent).toContain('1 object(s) in this site')
    app.unmount()
  }, 60000)

  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/c0c2fa4f-c38c-578e-8cc1-4703cc60b8a9')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Museum of Turkish and Islamic Arts'), { timeout: 20000 })
    expect(host.textContent).toContain('İstanbul')
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/c0c2fa4f-c38c-578e-8cc1-4703cc60b8a9/objects')
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-grid__tile').length).toBe(1), { timeout: 20000 })
    expect(host.textContent).toContain('Museum of Turkish and Islamic Arts')
    app.unmount()
  }, 60000)
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    // page on this site actually reads is listed, not just this gallery's
    // own — a raw shared key otherwise passes unseen (metanull/carpets#40):
    // the record/sheet/timeline/partner pages read the neutral namespaces
    // directly, on top of `gallery`, this site's own product section.
    expect(checkTextsRendered(host, {
      namespaces: ['communication', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
